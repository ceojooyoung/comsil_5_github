...
void LinkedList<T>::Print(){
	if (first == 0) return;			// 리스트가 비었으면 종료
	Node<T> *cur = first;			// first 노드를 저장
	for(int i=1;i<=current_size;i++){// 리스트 크기만큼 반복문 실행
		if(i!=1){ cout << "->";}				// 첫 노드가 아니라면 "->" 먼저 출력
		cout << "[" << i << "|" << cur->data << "]";	// i번째 원소 출력
		cur = cur->link;				//다음 노드 이동
	}
	cout << endl;							//줄바꿈
}
